...
class ExampleTest extends TestCase
{
    ...
    public function view()
    {
        ...
        //0.カートに何も入れていない状態、
        //echo "カートに何も入れていない状態場合,'お客様のショッピングカートに商品はありません。'を出力";
        ...
        //echo "正しい場合：";
        $cart->init();
        $element = new Element('Amazon Web Services 業務システム設計・移行ガイド', 2000);
        $element->setQuantity(4);
        $cart->add($element);

        $element = new Element('プログラマの数学第2版', 1500);
        $element->setQuantity(2);
        $cart->add($element);

        $expected = "Amazon Web Services 業務システム設計・移行ガイド\t2000\t4".PHP_EOL
                   ."プログラマの数学第2版\t1500\t2".PHP_EOL
                   ."小計 (6 点): \\11000";
